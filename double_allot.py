...
Phase 2 : SA optimises lab slot assignment with theory slots FIXED.
          Objective is the same — minimise unsolvable students —
          but the full conflict check (T-T, L-L, T-L, self-overlap)
          is used for scoring.
Phase 3 : Per-student optimal section assignment.

Conflict types checked:
  T-T  : two theory slots overlap
  T-L  : a theory slot overlaps a DIFFERENT course's lab slot
  self : a theory course's own lab must not overlap its theory slot
         (hard constraint — invalid moves are rejected outright)
  L-L  : two lab slots overlap
"""
...
# ══════════════════════════════════════════════════════════════════
# SHARED CONFLICT-FREE CHECK  (theory + labs)
# ══════════════════════════════════════════════════════════════════
        ...
        if not conflict:
            for ti, tc in enumerate(tcourses):
                ...
                if lab_code and lab_code in lcourses:
                    ...
                    if tl_conflict(t_slots[ti], l_slots[li]):
                        conflict = True; break
...
# ══════════════════════════════════════════════════════════════════
# PHASE 1 — THEORY SA
# (objective + cache use theory-only conflict check for speed)
# ══════════════════════════════════════════════════════════════════
...
# ── lab enrollment weight (for lab SA) ────────────────────────
...
def run_theory_sa(start_asgn, iterations, T_start, T_end,
    ...
    for it in range(iterations):
        ...
        old_m, old_e = current[c]
        if r < 0.40:
            new_m, new_e = random.choice(m_opts), old_e
        elif r < 0.80:
            new_m, new_e = old_m, random.choice(e_opts)
        else:
            new_m, new_e = random.choice(m_opts), random.choice(e_opts)
...
# ══════════════════════════════════════════════════════════════════
# PHASE 2 — LAB SA
# Theory slots are FIXED. Objective uses the full conflict check.
# ══════════════════════════════════════════════════════════════════

def build_lab_unsolvable_cache(t_asgn, l_asgn):
    """
    Build the set of students who have no conflict-free section combo
    given the current theory + lab assignment.
    Uses the full check (T-T, L-L, T-L, self-overlap).
    """
    unsolvable = set()
    for s in students:
        sid = s["student_id"]
        # skip students with no lab courses — theory-only conflicts
        # are already 0 after Phase 1, so they won't appear here
        if not student_lab_courses[sid]:
            continue
...
def delta_move_lab(t_asgn, l_asgn, lab_code, new_m, new_e, unsolvable_set):
    """
    Compute the change in unsolvable-student count if lab_code is moved
    from its current (morning, evening) bundle to (new_m, new_e).
    Theory slots are fixed throughout.

    Returns (delta, affected_sids) without mutating l_asgn permanently.
    delta > 0  → move makes things worse
    delta < 0  → move makes things better
    """
    old_m, old_e = l_asgn[lab_code]
    if new_m == old_m and new_e == old_e:
        return 0, set()

    # Only students enrolled in this lab are affected
    affected = set(
        s["student_id"] for s in students
    ...
    for sid in affected:
        ...
        was_unsolvable = sid in unsolvable_set
        if was_unsolvable and solvable:
            gained += 1
        elif not was_unsolvable and not solvable:
            lost   += 1
...
def random_lab_assign(t_asgn):
    """
    Random initial lab assignment.
    Hard-rejects any bundle that overlaps the course's own theory slot.
    """
    l_asgn = {}
    for lab_code in lab_courses:
        theory_code = lab_to_theory.get(lab_code)
        if theory_code not in t_asgn:
            continue
...
def run_lab_sa(t_asgn, start_l_asgn, iterations, T_start, T_end,
    ...
    """
    SA over lab slot assignments with theory slots fixed.

    Move operator (mirrors theory SA):
      40% — change morning bundle only
      40% — change evening bundle only
      20% — change both

    Hard constraint: never assign a lab bundle that overlaps
    the course's own theory slot (invalid moves are skipped).

    Courses are sampled weighted by enrollment count so
    high-impact labs get perturbed more often.
    """
    ...
    for it in range(iterations):
        ...
        # Pick a lab course weighted by enrollment
        ...
        # Valid options — hard-reject self-overlapping bundles
        m_valid = [b for b in LAB_MORNING_BY_P.get(p, []) if not tl_conflict(tm, b)]
        e_valid = [b for b in LAB_EVENING_BY_P.get(p, []) if not tl_conflict(te, b)]
        if not m_valid or not e_valid:
            continue
...
# ══════════════════════════════════════════════════════════════════
# PHASE 2 — LAB SA  (theory slots fixed)
# ══════════════════════════════════════════════════════════════════
...
# Initial random lab assignment (self-overlap filtered)
...
# Verify self-overlaps (should always be 0 — hard constraint during moves)
self_overlaps = sum(
    1
...
for s in students:
    sid            = s["student_id"]
    combo, clashes = best_section_for_student(sid, final_t, final_l)
    student_results[sid] = {
        "name":      s["name"],
        "clashes":   clashes,
        "selection": combo,
