...
"""
FFCS VIT Chennai — Two-Section SA (Theory + Lab, Two-Phase)
Fixed: LAB_SLOT_MAP now maps each lab slot to its own period,
       with correct lab-specific time windows.
"""
...
# ══════════════════════════════════════════════════════════════════
# THEORY PERIOD TIMES (minutes from midnight)
# ══════════════════════════════════════════════════════════════════

THEORY_PERIOD_TIMES = {
    1:(480,530),   # 8:00–8:50
    2:(535,585),   # 9:00–9:50  (note: 9:00 not 8:51)
    3:(590,640),   # 10:00–10:50
    4:(705,750),   # 11:45–12:30 (post-break)
    5:(755,805),   # 12:35–1:25
    6:(840,890),   # 2:00–2:50
    7:(895,945),   # 3:00–3:50
    8:(950,1000),  # 4:00–4:50  (approx)
    9:(1005,1055),
    10:(1060,1110),
    11:(1115,1165)
}
...
# ══════════════════════════════════════════════════════════════════
# LAB PERIOD TIMES (separate from theory — lab hours are different)
# Morning lab slots: 8:00-8:50, 8:51-9:40, 9:51-10:40, 10:41-11:30, 11:40-12:30, 12:31-1:20
# Evening lab slots: 2:00-2:50, 2:51-3:40, 3:51-4:40, 4:41-5:30, 5:40-6:30, 6:31-7:20
# ══════════════════════════════════════════════════════════════════

LAB_PERIOD_TIMES = {
    # Morning
    "M1": (480, 530),   # 8:00–8:50
    "M2": (531, 580),   # 8:51–9:40
    "M3": (591, 640),   # 9:51–10:40
    "M4": (641, 690),   # 10:41–11:30
    "M5": (700, 750),   # 11:40–12:30
    "M6": (751, 800),   # 12:31–1:20
    # Evening
    "E1": (840, 890),   # 2:00–2:50
    "E2": (891, 940),   # 2:51–3:40
    "E3": (951, 1000),  # 3:51–4:40
    "E4": (1001, 1050), # 4:41–5:30
    "E5": (1060, 1110), # 5:40–6:30
    "E6": (1111, 1160), # 6:31–7:20
}

# ══════════════════════════════════════════════════════════════════
# LAB SLOT MAP — each lab slot gets its own unique period
# L1..L30  = morning slots (M1..M6 across days)
# L31..L60 = evening slots (E1..E6 across days)
# ══════════════════════════════════════════════════════════════════
...
LAB_SLOT_MAP = {
    ...
    # Tuesday morning
    "L7":  ("TUE", "M1"), "L8":  ("TUE", "M2"),
    "L9":  ("TUE", "M3"), "L10": ("TUE", "M4"),
    "L11": ("TUE", "M5"), "L12": ("TUE", "M6"),
    # Wednesday morning
    "L13": ("WED", "M1"), "L14": ("WED", "M2"),
    "L15": ("WED", "M3"), "L16": ("WED", "M4"),
    # Thursday morning
    "L19": ("THU", "M1"), "L20": ("THU", "M2"),
    "L21": ("THU", "M3"), "L22": ("THU", "M4"),
    "L23": ("THU", "M5"), "L24": ("THU", "M6"),
    # Friday morning
    "L25": ("FRI", "M1"), "L26": ("FRI", "M2"),
    "L27": ("FRI", "M3"), "L28": ("FRI", "M4"),
    "L29": ("FRI", "M5"), "L30": ("FRI", "M6"),
    # Monday evening
    "L31": ("MON", "E1"), "L32": ("MON", "E2"),
    "L33": ("MON", "E3"), "L34": ("MON", "E4"),
    "L35": ("MON", "E5"), "L36": ("MON", "E6"),
    # Tuesday evening
    "L37": ("TUE", "E1"), "L38": ("TUE", "E2"),
    "L39": ("TUE", "E3"), "L40": ("TUE", "E4"),
    "L41": ("TUE", "E5"), "L42": ("TUE", "E6"),
    # Wednesday evening
    "L43": ("WED", "E1"), "L44": ("WED", "E2"),
    "L45": ("WED", "E3"), "L46": ("WED", "E4"),
    "L47": ("WED", "E5"), "L48": ("WED", "E6"),
    # Thursday evening
    "L49": ("THU", "E1"), "L50": ("THU", "E2"),
    "L51": ("THU", "E3"), "L52": ("THU", "E4"),
    "L53": ("THU", "E5"), "L54": ("THU", "E6"),
    # Friday evening
    "L55": ("FRI", "E1"), "L56": ("FRI", "E2"),
    "L57": ("FRI", "E3"), "L58": ("FRI", "E4"),
...
# ══════════════════════════════════════════════════════════════════
# LAB BUNDLE CONSTRUCTION
# ══════════════════════════════════════════════════════════════════

def _build_lab_pairs():
    """Consecutive same-day lab slots form a P=2 bundle."""
    all_slots = sorted(LAB_SLOT_MAP.keys(), key=lambda x: int(x[1:]))
    morning_pairs, evening_pairs = [], []
    i = 0
    while i < len(all_slots) - 1:
        s1, s2 = all_slots[i], all_slots[i+1]
        n1, n2 = int(s1[1:]), int(s2[1:])
        d1, p1 = LAB_SLOT_MAP[s1]
        d2, p2 = LAB_SLOT_MAP[s2]
        # consecutive numbers AND same day
        if n2 == n1 + 1 and d1 == d2:
            pair = f"{s1}+{s2}"
            (morning_pairs if n1 <= 30 else evening_pairs).append(pair)
            i += 2
        else:
...
def _build_p4_bundles(pairs):
    """P=4 bundles = two P=2 pairs on different days."""
    bundles = []
    for p1, p2 in combinations(pairs, 2):
        d1 = LAB_SLOT_MAP[p1.split("+")[0]][0]
        d2 = LAB_SLOT_MAP[p2.split("+")[0]][0]
...
def _lab_windows(bundle_str):
    """Each lab slot gets its own time window from LAB_PERIOD_TIMES."""
    windows = []
    for slot in bundle_str.split("+"):
        day, period_key = LAB_SLOT_MAP[slot]
        s, e = LAB_PERIOD_TIMES[period_key]
        windows.append((day, s, e))
    return windows
...
TT_OVERLAP = {
    (a, b): _windows_overlap(ALL_THEORY_WINDOWS[a], ALL_THEORY_WINDOWS[b])
    for a in ALL_THEORY_SLOTS for b in ALL_THEORY_SLOTS
}
TL_OVERLAP = {
    (t, l): _windows_overlap(ALL_THEORY_WINDOWS[t], ALL_LAB_WINDOWS[l])
    for t in ALL_THEORY_SLOTS for l in ALL_LAB_BUNDLES
}
LL_OVERLAP = {
    (a, b): _windows_overlap(ALL_LAB_WINDOWS[a], ALL_LAB_WINDOWS[b])
    for a in ALL_LAB_BUNDLES for b in ALL_LAB_BUNDLES
}
...
# ══════════════════════════════════════════════════════════════════
# CONFLICT CHECK — used in SA objective
# ══════════════════════════════════════════════════════════════════

def count_combo_clashes(tcourses, lcourses, t_slots, l_slots):
    """Count physical time clashes for a given slot assignment combo."""
    clashes = 0
    nt, nl = len(tcourses), len(lcourses)
    ...
    for i in range(nt):
        for j in range(i+1, nt):
            if tt_conflict(t_slots[i], t_slots[j]):
                clashes += 1

    # lab vs lab
    for i in range(nl):
        for j in range(i+1, nl):
            if ll_conflict(l_slots[i], l_slots[j]):
                clashes += 1

    # theory vs lab — skip own paired course
    for ti, ts in enumerate(t_slots):
        for li, ls in enumerate(l_slots):
            if lab_to_theory.get(lcourses[li]) == tcourses[ti]:
                continue
            if tl_conflict(ts, ls):
                clashes += 1
...
def student_has_conflict_free_option(sid, t_asgn, l_asgn):
    ...
    for combo in product(*[[0, 1]] * (nt + nl)):
        t_slots = [t_opts[i][combo[i]]      for i in range(nt)]
        l_slots = [l_opts[i][combo[nt + i]] for i in range(nl)]
        if count_combo_clashes(tcourses, lcourses, t_slots, l_slots) == 0:
            return True
    return False
...
def build_theory_unsolvable_cache(asgn):
    ...
    for s in students:
        ...
        for combo in product(*[[0, 1]] * n):
            slots = [opts[i][combo[i]] for i in range(n)]
            if not any(tt_conflict(slots[i], slots[j])
                       for i in range(n) for j in range(i+1, n)):
                break
        else:
            unsolvable.add(sid)
...
def delta_move_theory(asgn, course, new_m, new_e, unsolvable_set):
    ...
    if new_m == old_m and new_e == old_e:
        return 0, set()
    affected = {s["student_id"] for s in students
                if course in student_theory_courses[s["student_id"]]}
    asgn[course] = (new_m, new_e)
    gained = lost = 0
    for sid in affected:
        ...
        solvable = any(
            not any(tt_conflict(
                        [opts[i][combo[i]] for i in range(n)][i],
                        [opts[i][combo[i]] for i in range(n)][j])
                    for i in range(n) for j in range(i+1, n))
            for combo in product(*[[0, 1]] * n)
        )
        was, now = sid in unsolvable_set, not solvable
        if was and not now: gained += 1
        elif not was and now: lost += 1
    asgn[course] = (old_m, old_e)
    return lost - gained, affected
...
def run_theory_sa(start_asgn, iterations, T_start, T_end,
    ...
    for it in range(iterations):
        ...
        if it - last_best_iter >= reheat_thresh and T < 1.0 and n_reheats < 5:
            ...
            if remaining > 1:
                temps[it:] = geometric_schedule(reheat_T, T_end, remaining)
                T = temps[it]; n_reheats += 1; last_best_iter = it
                print(f"    [{tag}] iter {it+1} → REHEAT #{n_reheats} T={reheat_T:.1f} best={best_r}")

        c = random.choices(courses_list,
                           weights=[theory_weighted_degree.get(x, 1) for x in courses_list])[0]
        sess   = get_sessions(c)
        m_opts = MORNING_BY_SESSIONS.get(sess, [])
        e_opts = EVENING_BY_SESSIONS.get(sess, [])
        if not m_opts or not e_opts: continue
        ...
        if delta < 0 or random.random() < math.exp(-delta / max(T, 1e-10)):
            ...
            real = len(unsolvable)
            if real < best_r:
                best_r = real; best_a = dict(current); last_best_iter = it
                if best_r == 0:
                    print(f"    [{tag}] iter {it+1} → OPTIMAL — stopping early")
                    return best_a, best_r

        if (it+1) % 1000 == 0:
            print(f"    [{tag}] iter {it+1:6d} | T={T:7.3f} | "
                  f"unsolvable={len(unsolvable)} best={best_r} | {time.time()-t0:.1f}s")
...
def build_lab_unsolvable_cache(t_asgn, l_asgn):
    return {s["student_id"] for s in students
            if student_lab_courses[s["student_id"]]
            and not student_has_conflict_free_option(s["student_id"], t_asgn, l_asgn)}
...
def run_lab_sa(t_asgn, start_l_asgn, iterations, T_start, T_end,
    ...
    for it in range(iterations):
        ...
        lab_code    = random.choices(lab_list,
                                     weights=[lab_enrollment.get(x,1) for x in lab_list])[0]
        theory_code = lab_to_theory.get(lab_code)
        if theory_code not in t_asgn: continue
        p       = get_lab_p(lab_code)
        tm, te  = t_asgn[theory_code]
        ...
        delta, affected = delta_move_lab(t_asgn, current, lab_code, new_m, new_e, unsolvable)
        if delta < 0 or random.random() < math.exp(-delta / max(T, 1e-10)):
            current[lab_code] = (new_m, new_e)
            for sid in affected:
                solvable = student_has_conflict_free_option(sid, t_asgn, current)
...
asgn     = random_theory_assign()
p1_u     = len(build_theory_unsolvable_cache(asgn))
print(f"[P1] Theory courses: {len(asgn)} | Initial unsolvable: {p1_u}/{N_STUDENTS}\n")

T_START = 200.0; T_END = 0.01; ITERS = 10_000; N_RESTARTS = 4
best_t = dict(asgn); best_t_score = p1_u
...
for restart in range(N_RESTARTS):
    ...
    if restart > 0:
        frac = [0.15, 0.25, 0.35][restart % 3]
        for c in random.sample(list(start.keys()), max(1, int(len(start)*frac))):
            sess = get_sessions(c)
            m_opts = MORNING_BY_SESSIONS.get(sess, [])
            e_opts = EVENING_BY_SESSIONS.get(sess, [])
            if m_opts and e_opts:
                start[c] = (random.choice(m_opts), random.choice(e_opts))
        print(f"  [R{restart+1}] Perturbed ({frac:.0%}): {len(build_theory_unsolvable_cache(start))} unsolvable")

    ra, rs = run_theory_sa(start, ITERS, T_START, T_END, tag=f"R{restart+1}",
                           reheat_thresh=6000, reheat_T_frac=0.25)
    if rs < best_t_score:
        best_t_score = rs; best_t = dict(ra)
    print(f"  [R{restart+1}] unsolvable={rs} global_best={best_t_score}")
    if best_t_score == 0:
        print("  [P1] Optimal — skipping remaining restarts"); break
...
init_l   = random_lab_assign(final_t)
init_l_u = len(build_lab_unsolvable_cache(final_t, init_l))
print(f"[P2] Lab courses: {len(init_l)} | Initial unsolvable: {init_l_u}/{N_STUDENTS}\n")

best_l = dict(init_l); best_l_score = init_l_u
...
for restart in range(LAB_N_RESTARTS):
    ...
    if restart > 0:
        frac = [0.20, 0.40, 0.60][restart % 3]
        for lc in random.sample(list(start_l.keys()), max(1, int(len(start_l)*frac))):
            tc = lab_to_theory.get(lc)
            p  = get_lab_p(lc)
            tm, te = final_t[tc]
            mv = [b for b in LAB_MORNING_BY_P.get(p, []) if not tl_conflict(tm, b)]
            ...
            if mv and ev:
                start_l[lc] = (random.choice(mv), random.choice(ev))
        print(f"  [LR{restart+1}] Perturbed ({frac:.0%}): {len(build_lab_unsolvable_cache(final_t, start_l))} unsolvable")

    ra, rs = run_lab_sa(final_t, start_l, LAB_ITERS, LAB_T_START, LAB_T_END,
                        tag=f"LR{restart+1}", reheat_thresh=4000, reheat_T_frac=0.30)
    if rs < best_l_score:
        best_l_score = rs; best_l = dict(ra)
    print(f"  [LR{restart+1}] unsolvable={rs} global_best={best_l_score}")
    if best_l_score == 0:
        print("  [P2] Optimal — skipping remaining restarts"); break
...
print("\n[Phase 3] Computing optimal section assignments...")

def best_section_for_student(sid):
    tcourses = [c for c in student_theory_courses[sid] if c in final_t]
    lcourses = [c for c in student_lab_courses[sid]    if c in final_l]
    nt, nl   = len(tcourses), len(lcourses)
    if nt + nl == 0:
        return {}, 0

    t_opts = [(final_t[c][0], final_t[c][1]) for c in tcourses]
    l_opts = [(final_l[c][0], final_l[c][1]) for c in lcourses]
    ...
    for combo in product(*[[0, 1]] * (nt + nl)):
        t_slots = [t_opts[i][combo[i]]      for i in range(nt)]
        l_slots = [l_opts[i][combo[nt + i]] for i in range(nl)]
        clashes = count_combo_clashes(tcourses, lcourses, t_slots, l_slots)
        if clashes < best_clashes:
            best_clashes = clashes
            best_combo   = combo
        if best_clashes == 0:
    ...
    t_combo = best_combo[:nt]
    l_combo = best_combo[nt:]
    result  = {}
    for i, c in enumerate(tcourses):
        result[c] = {
            "type":    "theory",
            "section": "morning" if t_combo[i] == 0 else "evening",
            "slot":    t_opts[i][t_combo[i]]
        }
    for i, c in enumerate(lcourses):
        result[c] = {
            "type":    "lab",
...
for s in students:
    sid            = s["student_id"]
    combo, clashes = best_section_for_student(sid)
    student_results[sid] = {
        "name":      s["name"],
        "clashes":   clashes,
        "solvable":  clashes == 0,
...
n_solvable = sum(1 for v in student_results.values() if v["solvable"])
print(f"[Phase 3] Zero-conflict: {n_solvable}/{N_STUDENTS}")
...
output = {
    "timetable":     {c: {"morning": m, "evening": e} for c, (m, e) in final_t.items()},
    "lab_timetable": {c: {"morning": m, "evening": e} for c, (m, e) in final_l.items()},
    "metrics": {
        "total_theory_courses": len(final_t),
        "total_lab_courses":    len(final_l),
        "total_students":       N_STUDENTS,
        "full_unsolvable":      N_STUDENTS - n_solvable,
        "solvable_students":    n_solvable,
        "solvable_pct":         round(n_solvable / N_STUDENTS * 100, 2),
        "section_distribution": {
            "pure_morning": 0, "pure_evening": 0, "mixed": 0
        }
    },
    "student_sections": {
        sid: {
            ...
            "solvable":  v["solvable"],
            "selection": {
                c: {"type": info["type"], "section": info["section"], "slot": info["slot"]}
                for c, info in v["selection"].items()
            }
        }
        for sid, v in student_results.items()
...
print("\n[DONE] Saved → data/two_section_timetable.json")
print(f"       Solvable: {n_solvable}/{N_STUDENTS} ({output['metrics']['solvable_pct']}%)")